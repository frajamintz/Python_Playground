...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # we're going to check that this is a correct value by trying to case
            # to an integer, and if it's not, then we say its invalid
            # of that spot is not available on the board, we also say its invalid
            try:
                val = int(square)
...
class  GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            ...
            # get the sqwuare based off the minimax algorithm
            square = self.minimax(game, self.letter) ['position']
        return square
    
    def minimax(self, state, player):
        max_player = self.letter # yourself
        other_player = 'O' if player == 'X' else 'X' # the other player OR whatever the letter the player is not
        ...
        for possible_move in state.available_moves():
            # step 1: mave a move, try that spot
            state.make_move(possible_move, player)
            # step 2: recurse using minimax to simulate a game after making taht move
